...
export const GRAZIA_VALUES: ValuePoint[] = [
  {
    ...
    anecdote: [
      ...
      "> Mio malgrado, è vero che non mi accontento mai, ma LETTERALMENTE per ogni cosa: in palestra voglio ottenere sempre di più, ogni volta aumento sempre di un chilo il peso per vedere se riesco a sopportarlo e se non accade la prima volta, non vado via finchè non riesco a fare almeno una serie anche di soli 3 ripetizioni con quel nuovo peso;",
      "> quando cucino un nuovo piatto lo riprovo più volte cambiando piccoli dettagli finchè non trovo la ricetta top che mi soddisfa (al costo di preparare la stessa cosa per più cene consecutive rendendomi antipatica a tavola);",
      "> a lavoro cerco di dare il meglio anche in cose che non ho mai fatto o che non mi vengono chieste poichè penso alla Grazia del futuro e vorrei che avesse quante più competenze trasversali possibili."
    ]
  },
  {
    id: 'excellence',
  ...
  },
  {
    ...
    anecdote: [
      ...
      "> Coraggio significa anche solo prendere un aereo completamente da sola e raggiungere una nuova città per la prima volta, oppure, caso peggiore, dire al proprio datore di lavoro che una cosa la sai fare anche se non hai la certezza effettiva che funzioni. Quando sono entrata qui in agenzia non avevo mai usato wordpress, sapevo che era più semplice e gestibile e che per sviluppare il mio primo sito da dipendente avrei potuto contare su una web designer senior con un esperienza ormai decennale. Completando il master però, la mia intenzione era scrivere codice e utilizzare uno dei framework che avevo studiato di recente.",
      "> La richiesta del mio datore era semplice: realizzare con qualsiasi tecnologia ritenevo opportuno un sito multi pagina con seo base e performance di lighthouse al top. Mi aveva dato carta bianca insomma. Mi sono alzata le maniche e mi sono messa a lavoro, era la mia opportunità per poter usare Nextjs, per poter dimostrare che il mio approccio era innovativo e diverso rispetto a quello che l’agenzia aveva proposto fino ad allora e garantire che sapevo gestire il tutto per un cliente reale. Dapprima ho dovuto studiare un pò più nel dettaglio il linguaggio, fino ad allora avevo solo sviluppato progettini da portfolio, non volevo fare brutte figure per il primo lavoro che mi veniva assegnato, ma credevo nelle mie capacità e sapevo che studiando un pò di più avrei potuto raggiungere l’obiettivo anche gestendo diversi problemi in itenere. Fortunatamente non ho lasciato che l’ansia prevaricasse ed è andata bene."
    ]
  },
   {
    id: 'proactivity',
    ...
    anecdote: [
      ...
      "> Ai tempi non avevo molta esperienza in ambito grafico, perciò nel tempo libero ho seguito una serie di corsi online che mi potessero dare un’infarinatura di quali strumenti venivano usati dalla maggior parte dei grafici per realizzare wireframe e layout. Ho imparato ad usare Figma e AdobeXD e ho capito alcune tecniche di UX/UI che potevano migliorare l’esperienza utente e che effettivamente, guardando al futuro della mia carriera, era bene sapere e mi avrebbero aiutato nei progetti futuri che avrei potuto fare da sola.",
